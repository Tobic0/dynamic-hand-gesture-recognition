...
def draw_options(_image, op=0):
    if op == 0:
        ...
        cv2.putText(_image, "Current action: "+actions[op-1], (5, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
    else:
                    ...
                    for frame_num in range(sequence_length):
                        ...
                        if frame_num == 0:
                            ...
                            cv2.putText(image, 'Collecting frames for {} Video Number {}'.format(actions[opt-1],
                                        sequence),(15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                            cv2.waitKey(2500)
                        else:
                            cv2.putText(image, 'Collecting frames for {} Video Number {}'.format(actions[opt-1],
                                        sequence), (15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                        ...
                        npy_path = os.path.join(PATH, actions[opt-1], str(len(os.listdir(PATH+"/"+actions[opt-1])) - 1), str(frame_num))
